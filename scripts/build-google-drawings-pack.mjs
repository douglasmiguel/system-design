...
const components = [
  ...
  ["12-sql-multiple", "SQL · Multiple", "Primary database with read replicas", "data", '<ellipse cx="144" cy="112" rx="49" ry="18"/><path d="M95 112v59c0 10 22 18 49 18s49-8 49-18v-59M207 147c0-10 22-18 49-18s49 8 49 18v53c0 10-22 18-49 18s-49-8-49-18zM95 145c0 10 22 18 49 18s49-8 49-18"/><path d="M193 151h-24m0 0 10-10m-10 10 10 10"/>'],
  ...
  ["18-pubsub-topic", "Pub/Sub Topic", "Fans each event out to subscribers", "async", '<circle cx="200" cy="147" r="31"/><circle cx="123" cy="105" r="17"/><circle cx="123" cy="204" r="17"/><circle cx="277" cy="105" r="17"/><circle cx="277" cy="204" r="17"/><path d="m139 114 37 22m-37 59 37-22m85-37 37-22m-37 59 37 22"/>'],
  ["19-message-queue", "Message Queue", "Buffers work for competing consumers", "async", '<path d="M103 111h194v79H103z"/><path d="M135 150h130m-31-31 31 31-31 31"/><path d="M155 212h90"/>'],
...
function componentSvg([slug, name, description, group, icon]) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1140" viewBox="0 0 400 380" role="img" aria-labelledby="title desc">
  <title id="title">${name}</title><desc id="desc">${description}</desc>
  <rect x="18" y="18" width="364" height="344" rx="20" fill="#fff" stroke="#cbd5e1" stroke-width="2"/>
  <g fill="none" stroke="${accent}" stroke-width="7" stroke-linecap="round" stroke-linejoin="round">${icon}</g>
  <path d="M42 278h316" stroke="${tint}" stroke-width="2"/>
  <text x="42" y="315" fill="#0f172a" font-family="Arial, Helvetica, sans-serif" font-size="28" font-weight="700">${name}</text>
  <text x="42" y="344" fill="#475569" font-family="Arial, Helvetica, sans-serif" font-size="17">${description}</text>
</svg>`;
}
...
Each component is a standard RGB PNG with a white card, an icon, a name, and a short description. Google does not import them as editable native shapes.
